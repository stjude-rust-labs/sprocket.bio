...
The Sprocket unit testing framework does not require any modifications to your source WDL, re-uses your [`run` configuration](/concepts/configuration), and everything required to test is done via YAML. The framework has been designed to get up and running with comprehensive unit tests as quickly and easily as possible.
