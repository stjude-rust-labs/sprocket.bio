...
symbolic import clauses on one line when they fit within
[`max_line_length`](/reference/configuration#format-max-line-length). Longer
expressions and clauses are split across lines.
...
::: info Note
Input sorting is disabled by default. To enable it, set
[`sort_inputs = true`](/reference/configuration#format-sort-inputs)
...
The formatter automatically adds trailing commas to multiline lists (for
example, `meta`, `parameter_meta`, and `runtime` sections). This is enabled by
default and can be configured with the
[`trailing_commas`](/reference/configuration#format-trailing-commas) option in
the `[format]` section of your `sprocket.toml`.
