...
GPU acceleration is not currently supported by the TES backend. If your workflow requires GPU resources, use the [Docker backend](/configuration/backends/docker) or an HPC backend ([LSF](/configuration/backends/lsf) or [Slurm](/configuration/backends/slurm)) with Apptainer.
